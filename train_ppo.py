...
base_env = GymWrapper(raw_env, device="cpu")
...
actor_net = nn.Sequential(
    ...
    nn.LazyLinear(num_cells, device=device),
    nn.Tanh(),
    # Output 1 score per button. No "2 *" multiplier.
...
policy_module = ProbabilisticActor(
    ...
    in_keys=["logits"],  # We only have logits now
    distribution_class=OneHotCategorical,  # Discrete distribution
...
replay_buffer = ReplayBuffer(
    storage=LazyTensorStorage(max_size=frames_per_batch, device="cpu"),
